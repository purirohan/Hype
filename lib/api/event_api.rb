...
module EventApi
	...
	def self.get_http_response(query)
		begin
	...
	def self.add_measure(data,attr)
		data.each do |d|
	...
	def self.image_it(imagee,average)
		i = 0
		imageNum = 1
		imagee.each do |s|
			if i == average
	...
	def self.count_it(countee)
		total = countee.count
		@average = total / 9
	end

	def self.sort_it(sortee,arg)
		sortee.sort! {|x,y| y[arg] <=> x[arg]} 
	end

	def self.find_overlaps
	...
	def self.replace_dups(data,type)
		if type == 'eventbrite'
			data.each do |event|
				event.each do |e|
					ran = rand_nums
